...
#         self.gait = TrotGaitGenerator()

#         # Velocity State Arrays
...
#         self.get_logger().info("Modularized Go2 Bridge Initialized Successfully!")

#     def cmd_vel_callback(self, msg):
#         if abs(msg.linear.x) > 0.01 or abs(msg.linear.y) > 0.01 or abs(msg.angular.z) > 0.01:
#             if self.nav_sm.state != 'MANUAL':
#                 self.get_logger().warn("Manual override detected! Returning to MANUAL state.")
...
#         cmd = msg.data.lower()
#         if 'return home' in cmd or 'home' in cmd:
#             self.nav_sm.state = 'RETURNING_HOME'
#         elif 'retrieve' in cmd:
#             self.nav_sm.state = 'LOCAL_SEARCH'
#             self.nav_sm.search_start_time = time.time()

#     def ball_detection_callback(self, msg):
#         self.nav_sm.target_x = msg.pose.position.x
#         self.nav_sm.target_y = msg.pose.position.y
#         self.nav_sm.state = 'WAYPOINT_APPROACH'
...
#                 if spotted and self.nav_sm.state == 'LOCAL_SEARCH':
#                     self.nav_sm.target_x, self.nav_sm.target_y = bx, by
#                     self.nav_sm.state = 'FINAL_INTERCEPT'

#                 # Compute State Velocity Parameters
#                 if self.nav_sm.state != 'MANUAL':
#                     self.target_velocity = self.nav_sm.compute_target_velocity(rx, ry, ryaw, spotted)
...
#                 # Gait Engine Torque Update
#                 self.gait.compute_and_apply_torques(self.model, self.data, self.current_velocity, time.time() - start_time)

#                 mujoco.mj_step(self.model, self.data)

#                 # --- 🎯 10CM FINE METRIC LOCAL COORDINATE SYSTEM 🎯 ---
#                 if viewer.user_scn:
#                     viewer.user_scn.ngeom = 0  # Reset overlay geoms
...
#                     axis_len = 3.0      # Total axis length (3.0 meters)
#                     axis_thick = 0.005  # Thin axis lines

#                     fwd_vec = R[:, 0]   # Local +X Vector (Forward)
#                     left_vec = R[:, 1]  # Local +Y Vector (Left)
#                     up_vec = R[:, 2]    # Local +Z Vector (Up)

#                     fwd_end = dog_pos + fwd_vec * axis_len
#                     left_end = dog_pos + left_vec * axis_len
#                     up_end = dog_pos + up_vec * 0.4  # Vertical Z indicator
...
#                     # 2. 10cm Granular Ticks & 0.5m Labels Loop
#                     # Loop from 0.1m (10cm) up to 3.0m (300cm)
#                     for step in range(1, 31):
#                         dist_m = step * 0.1  # Current distance in meters

#                         # --- +X (Forward Axis Ticks) ---
#                         fwd_pos = dog_pos + fwd_vec * dist_m
#                         fwd_pos[2] = 0.005  # Keep slightly above ground
                        
#                         # Render 10cm tick line across +X axis
#                         mujoco.mjv_connector(
#                             viewer.user_scn.geoms[viewer.user_scn.ngeom],
#                             type=mujoco.mjtGeom.mjGEOM_CYLINDER,
#                             width=0.003,
#                             from_=fwd_pos - left_vec * 0.04,
#                             to=fwd_pos + left_vec * 0.04
#                         )
#                         viewer.user_scn.geoms[viewer.user_scn.ngeom].rgba = np.array([1.0, 0.3, 0.3, 0.7])
#                         viewer.user_scn.ngeom += 1

#                         # Add explicit text labels every 0.5m step
#                         if step % 5 == 0:
...
#                                 pos=fwd_pos + np.array([0.0, 0.0, 0.03]),
...
#                         # Render 10cm tick line across +Y axis
#                         mujoco.mjv_connector(
#                             viewer.user_scn.geoms[viewer.user_scn.ngeom],
#                             type=mujoco.mjtGeom.mjGEOM_CYLINDER,
#                             width=0.003,
#                             from_=left_pos - fwd_vec * 0.04,
#                             to=left_pos + fwd_vec * 0.04
#                         )
...
#                                 type=mujoco.mjtGeom.mjGEOM_LABEL,
#                                 size=np.array([0.0, 0.0, 0.0]),
#                                 pos=left_pos + np.array([0.0, 0.0, 0.03]),
#                                 mat=np.eye(3).flatten(),
#                                 rgba=np.array([0.2, 1.0, 0.2, 1.0])
#                             )
#                             geom.label = f"+Y {dist_m:.1f}m"
#                             viewer.user_scn.ngeom += 1

#                     # 3. Ball Tracking Ray Line (if spotted)
#                     if spotted:
#                         mujoco.mjv_connector(
#                             viewer.user_scn.geoms[viewer.user_scn.ngeom],
#                             type=mujoco.mjtGeom.mjGEOM_CYLINDER,
#                             width=0.005, from_=dog_pos, to=np.array([bx, by, 0.05])
#                         )
